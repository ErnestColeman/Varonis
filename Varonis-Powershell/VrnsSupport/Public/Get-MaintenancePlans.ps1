...
CROSS APPLY (
   ...
   AND ss.freq_relative_interval = f.FrequencyRelativeInterval
	WHERE j.[name] LIKE '' + sp.[name] + '%') AS x"
        ...
        [System.Collections.ArrayList]$TaskArray = @()
    }
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess($SqlInstance, 'Retrieving maintenance plan xml')) {
            $MaintenancePlanXml = Invoke-sqlcmd $MaintenancePlanQry -SuppressProviderContextWarning -MaxCharLength 20000
        }
        ...
        if ($PSCmdlet.ShouldProcess($SqlInstance, 'Querying for backup details')) {
            ...
            foreach ($Plan in $MaintenancePlanXml) {
                ...
                Write-Verbose "[$((Get-Date).TimeOfDay) PROCESS]: List Tasks [ $SqlTaskData ]"
                
                foreach ($SqlTask in $SqlTaskData.Node) {

                    Write-Verbose "[$((Get-Date).TimeOfDay) PROCESS]: Assigning Task [ $SqlTask ]"
...
#                        ) {
                            ...
                            Write-Verbose "[$((Get-Date).TimeOfDay) PROCESS]: Created Item [ Created $TaskItem ]"

                            $TaskArray.Add($TaskItem) | Out-Null
                        #}
                    #}
                }
            }
