...
#
#Tested on: DatAdvantage 6.2.53, SQL 2012, Windows Server 2012r2
#
#-Requires PowerShell 3+ installed
#
##############################################################################


####UPDATE THESE VARIABLES
$filerList = "C:\Users\jonathan\Desktop\BulkAddFilers - 6.2.53v2\bulkAddFilers1.csv" 	#Location of filerList.csv
...
$serverCount = $filerInfo | Measure-Object | Select-Object -expand count #.length #Used to display progress counter
...
$filerInfo | foreach-object {
    ...
    if ([string]::IsNullOrEmpty($_.EventAgent)) {
         $IgnoreDriverChanges = $true #true = do not install agent
         }
         else {
             $IgnoreDriverChanges = $false #false = install agent
             }
    ...
    #enable/disable access denied events on folders
    if ([string]::IsNullOrEmpty($_.FolderAccessDeniedEvents )) {
                ...
                { 
                    "windows" {
                        ...
                        if ([string]::IsNullOrEmpty($collector)) { #if there is not a collector, do this
                            ...
                            $filer = New-WindowsFileServer -name $hostname -AddToFilteredUsers $true -FileWalkCredentials $fwcred -AgentCredentials $agentCred -ShadowSqlCredential $dbSQLCred -DBInstallCredential $dbServerCred -ProbeID $probeID –DiscoverShares TopLevelOnly -Force 
                        }
                        else {
                            " - Building fileserver object on collector: " + $collector
                        ...
                        if ($IgnoreDriverChanges) {
                            " -- Do not install event agent"
                            $filer.Config.All.IgnoreDriverChanges = $true # do not install the event agent
                        }
                        if ($filewalkAgent) {
                            " -- Do not install filewalk agent"
                            $filer.Config.All.SkipFileWalkChange = $true # do not install the filewalk agent
                            $filer.Config.All.FileWalkMethod = "NFS" # set the filewalk method to cifs
                        } 
                        if (!$collectEvents) {
                            " -- Disable event collection"
                            $filer.CollectEvents = $false
                        }    
                        ...
                        if ([string]::IsNullOrEmpty($collector)) { #if there is not a collector
                            " - Adding " + $hostname + " to Management Console queue"
                            $jid = Add-fileserver $filer -Force
                            " - Added " + $hostname + " to Management Console queue"
                        }    
                        else { #if there is a collector
                            " - Adding " + $hostname + " to Management Console queue"
                    ...
                    "emc" {
                        ...
                        if ([string]::IsNullOrEmpty($collector)){ #if there is not a collector, do this
                            ...
                            "- Building fileserver object"
                            $filer = New-CelerraFileServer -name $hostname -AddToFilteredUsers $true -FileWalkCredentials $fwcred -ShadowSqlCredential $dbSQLCred -DBInstallCredential $dbServerCred -ProbeID $probeID -Force #–DiscoverShares All
                            ...
                            $filer.Config.EmcCollectionMethod = "Cepa"
                            #$filer.Config.All.IsFwIncEnabled = $false #uncomment to disable Incremental Filewalk
                            "- Adding fileserver to Management Console queue"
                            $jid = Add-fileserver $filer -Force -AutoFillVolumes 
                            "- Added " + $hostname + " to Management Console queue"
                            }
                        else { #if there is a collector, do this
                            ...
                            #$filer.Config.All.IsFwIncEnabled = $false #uncomment to disable Incremental Filewalk
                            "- Adding fileserver to Management Console queue"
                            $jid = Add-fileserver $filer -CollectorCredential $collectorCred -Force -AutoFillVolumes
                            "- Added " + $hostname + " to Management Console queue"
                            }
                        }
                      ...
                      "sharepoint" {
                        ...
                        if ([string]::IsNullOrEmpty($collector)){ #if there is not a collector, do this
                            ...
                            "- Building fileserver object"
                            $filer = New-SharePointFileServer -name $hostname -AddToFilteredUsers $true -AgentCredentials $agentCred -FileWalkCredentials $fwcred -ShadowSqlCredential $dbSQLCred -DBInstallCredential $dbServerCred -ProbeID $probeID -URL "http://sp2013" -Force
                            $filer.Config.All.IgnoreDriverChanges = $IgnoreDriverChanges
                            #$filer.Config.All.CollectLocalAccountsInfo = $CollectLocalAccountsInfo
                            #$filer.Config.All.AccessDeniedEvents = $AccessDeniedEvents
                            #$filer.Config.All.OpenDirectoryAccessDeniedEvents = $OpenDirectoryAccessDeniedEvents 
                            ...
                            #$filer.Config.All.IsFwIncEnabled = $false #uncomment to disable Incremental Filewalk
                            "- Adding fileserver to Management Console queue"
                            $jid = Add-fileserver $filer -AutoFillVolumes -Force  
                            "- Added " + $hostname + " to Management Console queue"
                            }
                        else { #if there is a collector, do this
