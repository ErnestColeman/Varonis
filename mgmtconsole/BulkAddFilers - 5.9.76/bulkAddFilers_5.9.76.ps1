...
#
#Tested on: DatAdvantage 5.9.76, SQL 2012, Windows Server 2012r2, PowerShell 3
#
##############################################################################


####UPDATE THESE VARIABLES
$filerList = "C:\Users\Jonathan\Desktop\bulkAddFilers.csv" 	#Location of filerList.csv
                ...
                { 
                    "windows" {
                        ...
                        if ([string]::IsNullOrEmpty($collector)){ #if there is not a collector, do this
                            ...
                            $filer.Config.All.SkipFileWalkChange = $filewalkAgent
                            ...
                            #$filer.Config.All.IsFwIncEnabled = $false #uncomment to disable Incremental Filewalk
                            "- Adding fileserver to Management Console queue"
                            $jid = Add-fileserver $filer -AutoFillVolumes -CollectorCredential $collectorCred -Force
                            $progressCount + "- Added " + $hostname + " to Management Console queue"
                            }
                        }
                     ...
                     "unix" {
                        ...
                        if ([string]::IsNullOrEmpty($collector)){ #if there is not a collector, do this
                            "- Building fileserver object"
                            $filer = New-UnixFileServer -name $hostname -FileWalkCredentials $filerCred -ShadowSqlCredential $dbsqlcreds -DBInstallCredential $dbServerCred -DiscoverShares TopLevelOnly
                            $filer.Config.All.SavePropertiesHistory = $SavePropertiesHistory
                            $filer.Config.All.SharesAutoDetectionFashion = $SharesAutoDetectionFashion
                            "- Adding fileserver to Management Console queue"
                            ...
                            "- Building fileserver object"
                            $filer = New-UnixFileServer -name $hostname -FileWalkCredentials $filerCred -ShadowSqlCredential $dbsqlcreds -DBInstallCredential $dbServerCred -DiscoverShares TopLevelOnly -Collector $collectorID
                            $filer.Config.All.SavePropertiesHistory = $SavePropertiesHistory
                            $filer.Config.All.SharesAutoDetectionFashion = $SharesAutoDetectionFashion
                            "- Adding fileserver to Management Console queue"
                            $jid = Add-fileserver $filer -CollectorCredential $collectorCred -Force
