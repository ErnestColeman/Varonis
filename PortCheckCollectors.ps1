<#
.SYNOPSIS
    Test DSP to Collector(s) communication over required ports
.DESCRIPTION
    This scipt is used to test port connectivity (one-way) DSP -> Collector(s). 
.NOTES
    File Name      : PortCheckCollectors.ps1
    Author         : Ernest Coleman (ecoleman@varonis.com)
    Prerequisite   : Powershell 3 and up & run from the DSP server.
#>
